  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      form.parse(req, async (err: any, fields: Fields, files: Files) => {
        ...
        let file_id = Array.isArray(fields.file_id) ? fields.file_id[0] : fields.file_id;
        let new_file = Array.isArray(fields.new_file) ? fields.new_file[0] : fields.new_file;
        let student_id = Array.isArray(fields.student_id) ? fields.student_id[0] : fields.student_id;
        let consultation_type = Array.isArray(fields.consultation_type) ? fields.consultation_type[0] : fields.consultation_type;
        ...
        const recycleBinQuery = `
          ...
          VALUES (?, ?, ?, NOW())
        `;
        await pool.query(recycleBinQuery, [file_id, file.file_name, username]);
        ...
        const updateQuery = `
          ...
          WHERE id = ?
        `;
        const [updateResult] = await pool.query<ResultSetHeader>(updateQuery, [`upload/${newFileName}`, username, file_id]);
        ...
        `;
        await pool.query(historyQuery, [file_id, student_id, username, newFileName, consultation_type]);
