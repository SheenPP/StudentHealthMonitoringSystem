...
const Sidebar: React.FC = () => {
  ...
  return (
    <div
      className={`bg-gray-50 ${isCollapsed ? "w-24" : "w-64"} h-screen p-6 border-r border-gray-200 shadow-md flex flex-col transition-all duration-300 relative`}
    >
      {/* Collapse Button */}
      <button
        onClick={() => setIsCollapsed(!isCollapsed)}
        className="py-1 px-1 bg-white border-2 border-gray-400 absolute -right-4 top-6 text-gray-600 focus:outline-none rounded-full"
      >
...
const SidebarItem: React.FC<SidebarItemProps> = ({ href, icon, text, isCollapsed, active }) => {
  return (
