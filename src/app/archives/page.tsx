...
import useAuth from "../hooks/useAuth"; // 🔐 Your custom hook for checking user login
...
export default function Archives() {
  const { user, authChecked, loading: authLoading } = useAuth(); // ✅ Secure hook (uses cookie)
  ...
  }, [authChecked, user, fetchArchives, router]);

  const handleRestore = async (id: string, file_name: string) => {
    if (!window.confirm(`Are you sure you want to restore "${file_name}"?`)) return;
    setProcessingId(id);
    try {
      await axios.post(
        "/api/recycle-bin",
